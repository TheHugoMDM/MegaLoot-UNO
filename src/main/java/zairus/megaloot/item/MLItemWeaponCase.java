...
//        	stack.setTagCompound(tag);
		int active = tag.getInteger("mode");
		
	    tooltip.add(TextFormatting.GOLD + "Mode: "+TextFormatting.WHITE+mode[active]);
	}
	...
	public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand)
	{
		...
		//CHANGE LOOT TYPE (CREATIVE ONLY)
		if (player.isCreative() && player.isSneaking()) {
            ...
            player.sendStatusMessage(
            	    new TextComponentString("Drop: " + mode[active]),
            	    true // true = action bar, false = chat normal
            	);
            tag.setInteger("mode", active);
            itemStack.setTagCompound(tag);
			
		}else {
			//
			NBTTagCompound tagG = itemStack.getTagCompound();
			if (tagG == null) {
			    tagG = new NBTTagCompound();
			}
			int active = tagG.getInteger("mode");
			//
			world.playSound(
					(EntityPlayer)null
					, player.getPosition()
			...
			for (int i = 0; i < 1; ++i)
			{
				...
				if (active>0){
					loot = LootItemHelper.getSpecificLoot(active,itemRand, case_rarity);
				}
